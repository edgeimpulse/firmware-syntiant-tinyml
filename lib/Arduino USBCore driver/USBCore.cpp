...
{
	// MPW start
	if (config == (USB_ENDPOINT_TYPE_ISOCHRONOUS | USB_ENDPOINT_IN(0)))
	{
	...
	if (config == (USB_ENDPOINT_TYPE_ISOCHRONOUS | USB_ENDPOINT_OUT(0)))
	{
		...
		usbd.epBank0SetAddress(ep, &udd_ep_in_cache_buffer[ep]);
		usbd.epBank0SetType(ep, 2); // ISOCHRONOUS OUT
	}
	// MPW end
...
uint32_t USBDeviceClass::send(uint32_t ep, const void *data, uint32_t len)
{
	...
	while (len != 0)
	{
		...
		// MPW
		//if (len >= EPX_SIZE) {
		if ((len >= EPX_SIZE) && (USB->DEVICE.DeviceEndpoint[ep].EPCFG.bit.EPTYPE1 != 2))
		{
